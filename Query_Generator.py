...
class Generator:
    ...
    def update_query(self, old_code):
        query_dict = {}
        ex1 = "'"
        ex2 = "'"
        if (
            ...
            if self.oversize is not None and self.oversize != '':
                query_dict['oversize'] = ex1 + str(self.oversize) + ex2
            query = self.form_update_query(query_dict, ex1 + old_code + ex2)
        return query

    def form_update_query(self, query_dict, old_code):
        ...
        for key in query_dict:
            ...
            if element_counter == dict_size:
                query_part = (query_part + 'PISTONS.' + key + ' = ' + query_dict[key] + ' WHERE PISTONS.pistonCode = '
                              + str(old_code) + ';')
            element_counter = element_counter + 1
        query = query + query_part
        print(query)
